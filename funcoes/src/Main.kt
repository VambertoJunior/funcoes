...
fun mostrarNotas(materia:String){
    if(!materiasENotas.containsKey(materia)){
        ...
        if (listaNotas != null) {
            ...
            println("Não foi mostrar notas matéria ${materia}")
        }
        println()
    }
}
/*Retorna a média obtida apartir de uma lista de notas */
fun calcularMedia(vararg disciplinas: String): Double {
    var totalNotas = 0.0
    var totalDisciplinas = 0
    for (disciplina in disciplinas) {
        val notas = materiasENotas[disciplina]
        if (notas != null) {
            totalNotas += notas.sum()
            totalDisciplinas += notas.size
        }
    }
    return if (totalDisciplinas != 0) {
        totalNotas / totalDisciplinas
    } else {
        0.0
    }
}
fun adicionarVariasNotas(materia:String, vararg nota:Double): Boolean{
    ...
    return notasDaMateria.addAll(notas.toList())
}
fun main(){
    ...
    adicionarDisciplina(materia = "História")
    ...
    adicionarVariasNotas("Física", 6.5, 7.0, 8.0)
    // 10. mostrarNotas -> mostre as notas da disciplina que você acabou de criar;
    println("Mostrando as notas da disciplina de Física:")
    mostrarNotas("Física")
}
